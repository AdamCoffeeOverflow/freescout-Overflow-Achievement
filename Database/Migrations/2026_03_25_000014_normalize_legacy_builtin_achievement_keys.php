...
class NormalizeLegacyBuiltinAchievementKeys extends Migration
{
    public function up()
    {
        ...
        $rows = DB::table('overflowachievement_achievements')
            ->orderBy('id')
            ->get(['id', 'key', 'trigger', 'threshold', 'created_by']);
        ...
        foreach ($rows as $row) {
            ...
            $canonical = $this->canonicalKey($key, trim((string)$row->trigger), (int)$row->threshold);
